...
export class LocalMusicService {
  ...
  async refreshLibrary(
    ...
    // 尝试使用 Rust 扫描器
    const emi = loadNativeModule("external-media-integration.node", "external-media-integration") as EmiModule | null;
    ...
    for (let i = 0; i < entries.length; i += PROCESS_BATCH_SIZE) {
      ...
      const tasks = chunk.map((entry) => {
        return this.limit(async () => {
          ...
          const isRustEntry = (e: any): e is AudioMeta => 'duration' in e && typeof e.duration === 'number';
          ...
          if (isRustEntry(entry)) {
             ...
             mtime = stats.mtimeMs;
             size = stats.size;
             // 小于 1MB 的文件不处理 (Rust 扫描器内部已经处理了，但 FastGlob 需要这里处理)
